...
cache_dir = os.path.expanduser("~/data").replace('\\', '/')
cache_subdir = "hdspikes"
# get_shd_dataset(cache_dir, cache_subdir)

train_file = h5py.File(os.path.join(
    cache_dir, cache_subdir, 'shd_train.h5').replace('\\', '/'), 'r')
test_file = h5py.File(os.path.join(
    cache_dir, cache_subdir, 'shd_test.h5').replace('\\', '/'), 'r')
...
def sparse_data_generator_from_hdf5_spikes(X, y, batch_size, nb_steps, nb_units, max_time, shuffle=True):
    ...
    labels_ = np.array(y, dtype=np.compat.long)
    ...
    while counter < number_of_batches:
        ...
        y_batch = torch.tensor(labels_[batch_index], dtype=np.compat.long, device=device)
